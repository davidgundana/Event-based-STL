...
def barrier(State,pos, posStart,posRef, t, Ts, phi, tmax, hz):
    ...
    for i in range(0, np.size(phi)):
        ...
        if type == 'ev':
            ...
            if t >= a + inputTime and t <= b + inputTime:
                bxtTemp, phi[i] = evBarrier(State, phi[i], t, Ts, a, b, inputTime, pos, posRef, posStart, hz)
...
def evBarrier(State, phi, t, Ts, a, b, inputTime, pos, posRef, posStart, hz):
    p = phi.p
    funcOf = phi.funcOf
...
def alBarrier(State, phi, t, Ts, a, b, pos, posRef, tmax, unt):
    p = phi.p
    funcOf = phi.funcOf
...
def partials(State,pos, posStart, posRef, t, Ts, phi, tmax, hz,bxt_i):
    delt = .01
    newx = list(pos)
    bPartialX = []
    ...
    for i in range(0,np.size(pos)):
        newx[i] = newx[i] + delt
        btemp, phiTemp, actProp, beven = barrier(State,newx,posStart, posRef, t, Ts, phi, tmax, hz)
    ...
    for i in range(0,np.size(t)):
        newt = newt + delt
        btemp, phiTemp,actProp, beven = barrier(State,pos, posStart, posRef, newt, Ts, phi, tmax, hz)
